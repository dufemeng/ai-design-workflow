...
当前实现需要清洗：T0-T13 搭起了 ADW fallback 底座，但一度把部分 Impeccable skill 当成 CLI 能力描述，并用自写 detector / critique / DESIGN seed 顶替主路径，且四个闭环命令缺 CLI 入口（已由 T16 接线）。T14b 已补 handoff/import 边界，T17 已补 agent 编排入口；剩余清债重点是 T15 state/interaction driver 和 T18 设计稿 baseline diff。
...
因此，当前改造目标不是”补一个大 adapter”，而是先把能力分层改正确并接上断掉的闭环：**接线 flow 生命周期命令 + agent 编排入口 + agent skill handoff + callable detect adapter + ADW deterministic substrate**。这些 P0 底座已覆盖 T14a/T14b/T16/T17；T15/T18 继续作为运行期增强。
...
HTML design artifact
  -> Design 阶段权威可视化设计稿
  -> Code 阶段 gap 验证 baseline（T18 目标态；当前 T10 不做设计稿 baseline diff）
...
当前实现状态：T0-T13 实现了 ADW fallback MVP 的大部分模块（proposal / design / review / code context / gap / ledger / live workbench / PatchIntent）。闭环命令一度缺失，已由 MVP 任务 T16 接线，现可经 CLI 端到端跑通；detect adapter 已接入（T14a），handoff/import（T14b）和 agent 编排入口（T17）已落地；state/interaction driver（T15）与设计稿 baseline diff（T18）仍待做。详见 MVP 实施文档 §8/§9。
