...
因此，当前改造目标不是“补一个大 adapter”，而是先把能力分层改正确：**agent skill handoff + callable detect adapter + ADW deterministic substrate**。
...
ADW CLI substrate
  ...
  -> gap token / DOM / state-driver / report
...
| `gap:run` | 运行 token / DOM / state-driver / detector adapter；detector 来源只能是 `impeccable detect --json` |
...
2. Agent skill 结果的 import schema 要先支持哪些：`document`、`critique`、`polish`、`audit`、`live`。
