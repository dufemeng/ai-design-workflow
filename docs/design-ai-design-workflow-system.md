...
gap report
  设计产物与实现的差异证据
  当前能力 = 实现页健康 + token/detector 闸门 + state/interaction driver + 设计稿语义 baseline diff
```
...
当前实现需要清洗：T0-T13 搭起了 ADW fallback 底座，但一度把部分 Impeccable skill 当成 CLI 能力描述，并用自写 detector / critique / DESIGN seed 顶替主路径，且四个闭环命令缺 CLI 入口（已由 T16 接线）。T14b 已补 handoff/import 边界，T17 已补 agent 编排入口，T15 已补 state/interaction runtime driver，T18 已补设计稿语义 baseline diff。剩余重点转为真实项目降噪、role/selector 覆盖和视觉证据增强。
...
- **gap 名实不符已修复到语义层。** 当前 gap 已加载 `design-<flow>.html` 做 baseline diff，能发现设计稿关键文本在实现页缺失；尚未覆盖的是像素级视觉还原、完整 role tree 和高阶选择器 diff。
- **B 主流程缺编排入口（已修）。** Agent 主控需要一个具体可安装的 skill / workflow 入口；T17 已提供 `skills/adw-workflow` 与 `adw skill:install`。

因此，当前改造目标不是”补一个大 adapter”，而是先把能力分层改正确并接上断掉的闭环：**接线 flow 生命周期命令 + agent 编排入口 + agent skill handoff + callable detect adapter + ADW deterministic substrate**。这些 P0 底座已覆盖 T14a/T14b/T15/T16/T17/T18；后续增强集中在降噪、覆盖面和真实项目数据。
...
ADW CLI substrate
  ...
  -> gap token / DOM 健康 / detector adapter / state-interaction runtime driver / design baseline diff / report
...
| `gap:run` | 当前运行 token / DOM 健康 / detector adapter（detector 来源只能是 `impeccable detect --json`）；state / interaction 有 driver 时真实执行，缺 driver 标 `not-testable`；加载 `design-<flow>.html` 做设计稿语义 baseline diff |
...
adw import:<skill> <dir> <slug> <result.json>
  ...
  -> 按 skill schema 写入 draft / report / Flow Ledger / provenance artifact
  -> 对会改变 flow 状态的导入更新 resume pointer
```

`<skill>` ∈ `document | critique | polish | audit | live`。命令名、handoff context 字段、import result schema 的可执行规格见 MVP 任务 T14；MVP 已实现 `document` 与 `critique` 的专用 schema，其余 skill 先走通用 provenance 落档。
...
| state coverage | 设计门静态检查 + 运行期 state driver | 设计门阻塞；运行期有 driver 则执行，缺 driver 标 `not-testable` | 设计稿必须声明空、加载、错误、成功、边界数据；运行期通过 query-param、mock response、fixture、feature flag、test hook 等驱动验证 |
| DOM health / semantic diff | DOM health + 设计稿 baseline 语义 diff | DOM health 阻塞；高置信 baseline 缺失文本可阻塞 | 当前检查页面非空白、基本标题结构，并从设计稿 iframe `srcdoc` 抽取关键文本与实现页 DOM 比对；role tree / masked visual diff 属后续增强 |
| detector | `impeccable detect --json` | 阻塞 | ADW 不再保留自写 detector 主路径；detect 不可用时必须显式失败或标记未跑，不能伪装通过 |
| interaction diff | Playwright interaction driver | 有 driver 则执行；缺 driver 标 `not-testable` / `not-run` | 点击、输入、展开、滚动、键盘、安全区、状态切换；默认作为提醒项，稳定后再按场景升级阻塞 |
...
改造后，运行期 gap 的阻塞项是 token / DOM health / detector，其中 detector 来源必须是 `impeccable detect --json`。state 和 interaction 已支持 runtime driver：有 driver 时真实执行，缺 driver 或外部条件不足时标记 `not-testable`，不能伪装成通过。

需要写实的差距：当前 `gap:run` 已加载 `design-<flow>.html` 做语义 baseline diff，但还不是像素级视觉还原，也不是完整 role tree / selector diff。可对外宣称的是「实现页 vs DESIGN.md token + detector + runtime driver + 设计稿关键文本 baseline」，不能宣称任意页面像素级自动还原。
...
HTML design artifact
  -> Design 阶段权威可视化设计稿
  -> Code 阶段 gap 验证 baseline（当前已做关键文本语义 diff；视觉 diff 后续增强）
...
- 用 token / DOM health / detector 做最小运行期阻塞检查；state / interaction 有 driver 时真实运行，缺 driver 标 `not-testable`；a11y 先提醒；截图只作为人工佐证。
...
当前实现状态：T0-T13 实现了 ADW fallback MVP 的大部分模块（proposal / design / review / code context / gap / ledger / live workbench / PatchIntent）。闭环命令一度缺失，已由 MVP 任务 T16 接线，现可经 CLI 端到端跑通；detect adapter 已接入（T14a），handoff/import（T14b）、agent 编排入口（T17）、state/interaction driver（T15）和设计稿 baseline diff（T18）已落地。详见 MVP 实施文档 §8/§9。

- Proposal / Prototype：一句话需求到 2 到 3 个 HTML 方案。
- Design：生成需求级设计文档和正式 HTML 设计稿。
- Code：自动 gap loop + 人工 live 入口；token / DOM health / detector 作为运行期阻塞检查，state / interaction 有 driver 时运行，a11y 作为提醒项。
...
- 「gap = 设计产物 diff」必须限定当前能力边界：T18 已实现关键文本语义 baseline diff，但不是任意页面像素级自动还原。对外叙述必须与实现能力对齐，否则会重蹈 Impeccable 伪装的覆辙。
...
- 如果 gap report 只做 screenshot diff，会误把合理响应式差异当成错误，也会漏掉语义和交互问题；当前运行期阻塞检查从 token、DOM health 和 detector 开始，state / interaction 通过 driver 验证，a11y 先做提醒，截图只做证据。
...
- gap baseline 能否稳定对齐 HTML 设计稿和实现页面，需要通过 route、可比区域、选择器和 evidence 规范验证；运行期状态 / 交互覆盖已由 T15 提供最小 driver，后续仍需真实项目降噪。
...
3. ~~State / Interaction Driver 优先支持哪类驱动~~（已决：T15 支持 query-param、mock-response、fixture、feature-flag、test-hook；seed-data 记录为需外部预置）。
