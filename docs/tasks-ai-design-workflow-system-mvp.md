...
| --- | --- | --- | --- | --- |
| `createFlow` | `adw flow:create <dir> <slug> <title>` | ✓ 已实现（T16） | 有需求输入和项目上下文 | 创建 workflow ledger |
| `recordQuestionAnswer` | `adw proposal:answer <dir> <slug> <q> <a> [flags]` | ✓ 已实现（T16） | 当前在 Proposal 探索循环 | 更新假设和未解决分歧 |
...
| `approveDesign` | `adw design:approve <dir> <slug>` | ✓ 已实现（T16） | 审查门通过，用户确认 | 进入 Code |
...
| `markDone` | `adw flow:done <dir> <slug> [--accept-warnings]` | ✓ 已实现（T16） | 阻塞检查通过，用户接受剩余提醒 | 完成 flow |

> 「接线状态」列以本文档为准：四个曾缺失的命令（`flow:create` / `proposal:answer` / `design:approve` / `flow:done`）已由 **T16 接线**，闭环现可经 CLI 端到端跑通（`flow:create` → … → `flow:done`），不再需要手改 `workflow.json`。详见 T16。
...
- （T14a，已完成）删除 ADW 自写 detector 主路径：
...
进入 Code、不可收尾。这是早期「闭环已验证」overclaim 的根因，本任务把它接上。
...
- **skill 内容大纲**（SKILL.md 必含小节）：
  1. 何时用：三种入口（一句话需求 / 截图 / 改现有页）各自的起步命令。
  2. 推进协议：每步先 `adw flow:status` 读 `resumePointer`，再按 §5.3 契约表调对应命令。
  3. Impeccable handoff：何时 `handoff:<skill>` / `import:<skill>`（T14b 落地后启用）。
  4. workbench 决策回写：用户口述 → agent 调 `approve* / record*` 命令。
  5. 硬不变量：不手改 `workflow.json`、不后台改 `DESIGN.md`、detector 只来自 impeccable detect。
- **意图→命令序列**对照表（三种入口各一条 happy path 命令链）。
- **版本策略**：SKILL.md frontmatter 带 `adwSkillVersion`；`skill:install` 检测目标仓库已装版本，落后时提示 `--update`；版本与 ADW 包版本对齐，避免 skill 引用了不存在的命令。

验收：

- 干净仓库里 `adw skill:install` 后，agent 只读该 skill + 跑 `adw flow:status` 就能不靠记忆推进到下一步。
- skill 描述的每个命令都真实存在于 CLI（与 §5.3 契约表逐条对得上）；CI/`npm test` 加一条「skill 引用的命令都在 CLI switch 里」的校验。
...
已满足（T16 接线 + T14a detect adapter 完成后）：

- [x] 能经 CLI 从零创建 flow（`createFlow` / `flow:create`）。
- [x] 中断后能经 CLI 续跑（resume pointer + `flow:status`）。
- [x] 没有跳过 Proposal / Prototype 探索循环，且问答能落账（`recordQuestionAnswer` / `proposal:answer`）。
- [x] 用户在原型选择、正式设计批准（`approveDesign` / `design:approve`）、收尾（`markDone` / `flow:done`）处有可执行决策点。
